...
async function scrapeN11() {
    ...
    if (!vendorId) {
        console.error('❌ n11 vendor ID oluşturulamadı!');
        await browser.close();
        return;
    }
    ...
    for(const p of allProducts) {
        ...
        // Clean title
        let cleanTitle = p.title.replace(/(Hızlı Teslimat|Ücretsiz Kargo|Kargo Bedava|Süper Fırsat|Flaş İndirim|Sepette İndirim)/gi, '').trim();
        ...
        if(!pId) {
            const mockSpecs = catSlug === 'akilli-telefon' 
                ? { "Ekran": "6.7 inç AMOLED", "İşlemci": "Snapdragon 8 Gen 3", "Batarya": "5500 mAh", "Garanti": "2 Yıl" }
                : catSlug === 'bilgisayar-laptop'
                ? { "Ram": "16 GB DDR5", "İşlemci": "Intel Core i7", "Ekran": "15.6 inç FHD", "SSD": "512 GB" }
                : { "Güç": "2400W", "Renk": "Siyah/Gri", "Garanti": "2 Yıl", "Enerji Sınıfı": "A++" };
        ...
        if(pId) {
            ...
            if(!ep) {
                await supabase.from('product_prices').insert({
                    ...
                    vendor_id: vendorId,
                    price: p.price,
                    original_price: Math.floor(p.price * 1.08),
                    product_url: p.url,
